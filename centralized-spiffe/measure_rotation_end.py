...
def compute_rotation_timing(rotated_num):
    ...
    new bundle is PUT to the centralized repo. Covers prepare + activate +
...
def measure_rotation_end(rotated_num=None, server_count=None):
    ...
    Args:
        rotated_num: server number that was rotated (auto-detected if None).
        server_count: total servers in federation (inferred from workloads dir if None).

    Returns:
        dict with rotation/propagation/full_mesh timing info.

    Raises:
        RuntimeError if timing files not found (used by pollers).
    """
    workloads_dir = spire_utils.artefacts_dir() / "workloads"
