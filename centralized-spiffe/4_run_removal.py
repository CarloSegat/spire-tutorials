...
def restart_workloads_for_server(removed_num, max_server):
    ...
    loop once peers drop the bundle). Scoped to script 4 to keep other
    experiments free of periodic-send noise.
    """
    ...
    env["PORT"] = str(8082 + (removed_num * 6 - 5))
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: 4_run_removal.py <removed_num> <server_count>", file=sys.stderr)
        sys.exit(1)
