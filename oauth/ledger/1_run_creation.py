#!/usr/bin/env python3
"""Orchestrate initial creation of n-domain ledger-OAuth federation.

Brings up a fresh Hardhat node, compiles & deploys the OAuthMetadataStore
contract, then follows the same flow as the centralized variant.

Measures federation creation time:
  t_start: first contract registerDomain() call
...
def run_creation(n: int):
    ...
    # Hardhat lifecycle
    orch.kill_existing_hardhat()
    orch.start_hardhat_node()
    orch.compile_contract()
    orch.deploy_contract()

    # Boot Keycloaks, realms, workloads, listeners
