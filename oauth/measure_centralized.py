...
def read_epoch(name):
    """Read epoch value from centralized/epochs/{name}.epoch"""
    path = f"centralized/epochs/{name}.epoch"
    try:
        return float(open(path).read().strip())
    except FileNotFoundError:
        print(f"ERROR: {path} not found. Did you run run_centralized.sh?", file=sys.stderr)
        sys.exit(1)
    except ValueError:
...
def main():
    ...
    if not os.path.isdir("centralized/epochs"):
        print("ERROR: centralized/epochs/ directory not found. Run from oauth/ directory.", file=sys.stderr)
        sys.exit(1)

    # Read all epoch values
    ...
    # Prepare row
    row = [n, creation_s, addition_s, rotation_call_s, propagation_s, mesh_after_rotation_s, removal_s]
    ...
    with open(csv_path, "a", newline="") as f:
        writer = csv.writer(f)
        if not file_exists:
            writer.writerow(["n", "creation_s", "addition_s", "rotation_call_s", "propagation_s", "mesh_after_rotation_s", "removal_s"])
        writer.writerow(row)
